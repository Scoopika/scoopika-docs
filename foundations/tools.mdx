...
There are different types of tools that agents can call in Scoopika:

### API tools

These tools are defined from the platform with no code, and allows the agent to send HTTP requests to external APIs.

The response of the HTTP request is returned back to the agent to process it.
