...
This method does not return the actual session history (runs or messages), But it returns a [StoreSession object](/api-reference/store-session), to get the session's runs, refer to [getSessionRuns](/package/ts/client/store/get-session-runs).
...
const session = await client.store.getSession("SESSION_ID");

// Retrieve session, throw error if not found
const session = await client.store.getSession(
	"SESSION_ID",
	false
);
```
