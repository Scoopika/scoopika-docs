...
To create a session that belongs to a specific user just specify the `user_id` property when creating a new session using the [newSession method](/packages/ts/client/store/new-session).
...
<ResponseField name="sessions_ids" type="Promise<string[]>">
  ...
  - [getSession](/packages/ts/client/store/get-session): To get the session info.
  - [getSessionRuns](/packages/ts/client/store/get-session-runs): Get the runs (messages) of the session.
</ResponseField>
